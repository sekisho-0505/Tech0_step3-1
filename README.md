# 価格設定支援システム

本リポジトリは「AI実装ガイド.md」「価格設定支援システム_要件定義書_v2.0.md」をもとに構築された学習用プロジェクトです。Next.js + MUI フロントエンドと FastAPI バックエンド、Supabase(PostgreSQL) を想定した構成になっています。初学者の方向けに、ローカル開発を始めるための手順をまとめました。

## ディレクトリ構成

```
project-root/
├── frontend/        # Next.js 14 + TypeScript + MUI 5
├── backend/         # FastAPI 価格シミュレーション API
├── database/        # Supabase/PostgreSQL 用マイグレーション
└── docker-compose.yml
```

## 事前準備

- Node.js 20 系
- npm 10 以上
- Python 3.11
- (任意) Docker Desktop または Docker Engine 20 以上

> Docker を利用すると、依存関係を個別にインストールする必要がありません。未経験の場合は Docker 手順を推奨します。

## セットアップ手順（Docker 利用）

1. リポジトリ直下でコンテナを起動します。
   ```bash
   docker compose up -d --build
   ```
2. 起動後のアクセス先
   - フロントエンド: http://localhost:3000
   - バックエンド: http://localhost:8000/docs (Swagger UI)
   - PostgreSQL: localhost:5432 / DB名 `pricing`

3. 停止する場合は以下を実行します。
   ```bash
   docker compose down
   ```

## セットアップ手順（ローカル実行）

### 1. バックエンド

```bash
cd backend
python -m venv .venv
source .\.venv\Scripts\Activate.ps1
pip install -r requirements.txt
uvicorn app.main:app --reload --port 8000
```

### 2. フロントエンド

別ターミナルを開きます。

```bash
cd frontend
npm install
npm run dev -- --port 3000
```

ブラウザで http://localhost:3000 を開くと価格シミュレーション画面が表示されます。フォームに原価・粗利率などを入力し、「計算を実行」を押すとバックエンド API にリクエストが送られます。

## 環境変数

| 変数名 | 用途 | 設定例 |
| ------ | ---- | ------ |
| `NEXT_PUBLIC_API_BASE_URL` | フロントエンドから参照する API の URL | `http://localhost:8000` |
| `DATABASE_URL` (将来拡張) | FastAPI が接続する Supabase/PostgreSQL | `postgresql://postgres:postgres@localhost:5432/pricing` |

Docker 起動時は `NEXT_PUBLIC_API_BASE_URL` が自動で設定されます。ローカルで環境変数を指定したい場合は `frontend/.env.local` を作成し、上記の値を記入してください。

## データベース

`database/migrations/001_initial_schema.sql` に要件定義書ベースの初期スキーマを用意しています。Supabase SQL エディタまたは `psql` で適用してください。

```bash
psql postgresql://postgres:postgres@localhost:5432/pricing -f database/migrations/001_initial_schema.sql
```

## 機能概要

- **バックエンド API**: `/api/price-simulations/calculate`
  - 原価（円/kg）、目標粗利率（0〜90%）、任意の数量(kg)を入力として受け取り、推奨単価・粗利益・5種類の価格パターンを返します。
  - 単位はすべて円／円/kgで統一し、四捨五入で整数化しています。
  - 最低売価（原価 + 5% マージン）を下回る場合はガードフラグと日本語メッセージを返します。
- **フロントエンド UI**: 左カラムで入力、右カラムで結果表示。Zustand で結果状態とローディングを管理し、Zod + React Hook Form によるリアルタイムバリデーションを実装しています。

## よくある質問

- **API がエラーになる**: 粗利率が 90% を超えていないか確認してください。0〜90% の範囲で指定します。
- **表示が更新されない**: ブラウザの開発者ツールからネットワークエラーが出ていないか確認し、バックエンドが起動中かチェックしてください。
- **データベースに接続したい**: Supabase のプロジェクトを作成し、`.env` に `DATABASE_URL` 等を設定すると FastAPI から接続できるようになります。

## コードレビュー結果

### レビュー実施日
2025年11月8日

### レビュー対象
- AI実装ガイド.md
- 価格設定支援システム_要件定義書_v2.0.md
に基づいた実装コードのレビューと改善

### 実装状況の総評
✅ **全体的に要件定義書v2.0に沿った良好な実装**

要件定義書とAI実装ガイドに基づき、以下の重要な要件が適切に実装されています：
- Decimalを使用した精度保証
- 四捨五入（ROUND_HALF_UP）の正しい実装
- 単位の統一（円/kg）
- 最低価格ガードの実装
- TypeScriptでの型安全性
- Material UIによる統一されたUI

### 実装された機能

#### バックエンド（FastAPI）
- ✅ 価格計算APIエンドポイント (`/api/price-simulations/calculate`)
- ✅ Pydanticによる入力バリデーション
- ✅ Decimalによる高精度計算
- ✅ 5つの価格パターン生成（10%, 15%, 20%, 25%, 30%）
- ✅ 最低売価ガード（5%マージン）
- ✅ CORS設定
- ✅ ヘルスチェックエンドポイント

#### フロントエンド（Next.js + MUI）
- ✅ 左右2カラムレイアウト
- ✅ React Hook Formによるフォーム管理
- ✅ Zodによるバリデーション
- ✅ Zustandによる状態管理
- ✅ 商品名オートコンプリート
- ✅ スライダーによる粗利率入力
- ✅ リアルタイムバリデーション
- ✅ エラー通知（Snackbar）
- ✅ レスポンシブデザイン

### 今回のレビューで実施した改善

#### 1. 総粗利益の計算と表示を追加 ✅
**変更内容:**
- `PriceSimulationResponse`に`gross_profit_total`フィールドを追加
- バックエンドで数量が指定されている場合に総粗利益を計算
- フロントエンドで総粗利益をChipで表示

**該当ファイル:**
- `backend/app/schemas.py:51`
- `backend/app/main.py:71-74`
- `frontend/src/types/priceSimulation.ts:23`
- `frontend/src/components/PriceSimulationResult.tsx:53-55`

#### 2. エラーハンドリングの改善 ✅
**変更内容:**
- エラーコードを`INVALID_INPUT`から`VALIDATION_ERROR`に変更
- 要件定義書に沿ったエラーメッセージに修正
- 日本語エラーメッセージの追加

**該当ファイル:**
- `backend/app/main.py:59`
- `backend/app/schemas.py:49-52`

#### 3. バリデーションの強化 ✅
**変更内容:**
- バリデーション規則を定数化（`MARGIN_RATE_MAX`, `UNIT_COST_MAX`等）
- 要件定義書v2.0の制約値を明示的に定義
- フィールドにdescriptionを追加

**該当ファイル:**
- `backend/app/schemas.py:12-18`
- `backend/app/schemas.py:34-43`

#### 4. コメントとドキュメントの追加 ✅
**変更内容:**
- 全てのクラスに日本語ドキュメント文字列を追加
- 計算ロジックに詳細なコメントを追加
- 関数に型ヒントとドキュメント文字列を追加

**該当ファイル:**
- `backend/app/schemas.py`: 全クラス・関数
- `backend/app/main.py:44-50`: エンドポイント

### 準拠状況

#### 要件定義書v2.0との対応

| 項目 | 状態 | 備考 |
|------|------|------|
| 計算式の正確性 | ✅ | `推奨価格 = 原価 / (1 - 粗利率)` を正しく実装 |
| 丸め処理 | ✅ | ROUND_HALF_UPで四捨五入 |
| 単位の統一 | ✅ | 内部は円/kg、整数で保存 |
| バリデーション範囲 | ✅ | 粗利率0〜90%、原価0.01〜999999999.999 |
| 最低売価ガード | ✅ | 5%マージンを最低ラインとして実装 |
| 5つの価格パターン | ✅ | 10%, 15%, 20%, 25%, 30% を提示 |
| 総粗利益計算 | ✅ | 数量指定時に計算・表示 |
| エラーメッセージ日本語化 | ✅ | 全エラーメッセージを日本語化 |

#### AI実装ガイドとの対応

| チェック項目 | 状態 | 備考 |
|-------------|------|------|
| Decimal型使用 | ✅ | 全ての金額計算でDecimal使用 |
| 四捨五入（ROUND_HALF_UP） | ✅ | round_jpy, round_rate関数で実装 |
| 単位統一（内部:円） | ✅ | 全て円単位で統一 |
| フロントエンドガード | ✅ | UIで最低価格チェック、保存ボタン無効化 |
| 構造化ログ | ⚠️ | 未実装（将来の拡張） |
| 厳密な型定義 | ✅ | TypeScript, Pydanticで完全型付け |
| テスト実装 | ⚠️ | 未実装（将来の拡張） |

### 未実装機能（将来の拡張ポイント）

#### 優先度：高
- データベース連携（Supabase）
- 価格シミュレーション結果の永続化
- 損益分岐点分析機能
- Excel取込機能

#### 優先度：中
- ユーザー認証（Supabase Auth）
- ロギング機能（構造化ログ）
- 単体テスト・統合テスト
- エラーバウンダリの実装

#### 優先度：低
- ダッシュボード画面
- 履歴検索機能
- データエクスポート機能
- パフォーマンス最適化

### 技術的な推奨事項

1. **テストの追加**: 要件定義書に記載されているテストケースの実装を推奨
2. **環境変数の管理**: `.env.example`ファイルの作成
3. **CI/CDパイプライン**: GitHub Actionsでのテスト自動化
4. **ロギング**: 本番環境向けの構造化ロギングの実装
5. **エラーモニタリング**: Sentryの導入

### コード品質指標

- **型安全性**: ⭐⭐⭐⭐⭐ (TypeScript + Pydantic)
- **可読性**: ⭐⭐⭐⭐⭐ (コメント充実、命名規則適切)
- **保守性**: ⭐⭐⭐⭐☆ (モジュール化、関心の分離)
- **テストカバレッジ**: ⭐☆☆☆☆ (未実装)
- **ドキュメント**: ⭐⭐⭐⭐⭐ (README、コメント充実)

### まとめ

本システムは要件定義書v2.0とAI実装ガイドに基づき、**高品質な実装**が行われています。特に以下の点が優れています：

1. **精度保証**: Decimalを使用した正確な金額計算
2. **型安全性**: TypeScript/Pydanticによる完全な型定義
3. **ユーザビリティ**: Material UIによる統一されたUI/UX
4. **バリデーション**: フロントエンド・バックエンド両方での厳格なチェック

今後は、データベース連携や認証機能の追加、テストの実装により、さらに堅牢なシステムへと発展させることができます。

学習用途の最初の一歩として、また実務レベルのコード品質を学ぶ教材として最適なプロジェクトです。
